...
import ChiTietPhieuChi from "./expense-detail/index.vue";
import ThanhToanThem from "./additional-payment/index.vue";
//Icon SVG
...
const loadPhieuChis = async () => {
  try {
    ...
    if (response && response.status) {
      expenseVouchers.value = response.data || [];
...
const calculateStatistics = () => {
  ...
  expenseVouchers.value.forEach((phieu) => {
...
const refreshStatistics = async () => {
  ...
  try {
    ...
    if (response && response.status) {
      const tempVouchers = response.data || [];
...
const expenses = computed(() => {
  return expenseVouchers.value.map((phieu) => ({
    id: phieu.id,
