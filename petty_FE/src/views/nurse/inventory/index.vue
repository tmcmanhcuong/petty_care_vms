...
import ThemThuocVatTu from "./add-medicine/index.vue";
import TheKho from "./inventory-card/index.vue";
import TaoPhieuNhap from "./create-receipt/index.vue";
import ChiTietPhieuNhap from "./receipt-detail/index.vue";
import ThemDoiTacCungCap from "./add-supplier/index.vue";
import SuaNhaCungCap from "./edit-supplier/index.vue";
import XoaNhaCungCap from "./delete-supplier/index.vue";
import DanhMucHangHoa from "./goods-category/index.vue";
...
const calculateStockQuantity = (hangHoaId) => {
  ...
  importRecords.value.forEach((record) => {
    ...
    const itemDetails = record._original?.chi_tiet_phieu_nhap_kho || [];
  ...
  try {
    ...
    if (response && response.status && Array.isArray(response.data)) {
      ...
      try {
        ...
        if (voucherResponse && voucherResponse.status) {
          // Xử lý cả trường hợp có pagination và không có pagination
          if (Array.isArray(voucherResponse.data)) {
            phieuChiData = voucherResponse.data;
          } else if (
            voucherResponse.data &&
            Array.isArray(voucherResponse.data.data)
          ) {
            phieuChiData = voucherResponse.data.data;
          }
      ...
      response.data.forEach((item) => {
        ...
        const filteredVouchers = phieuChiData.filter(
          (phieu) =>
